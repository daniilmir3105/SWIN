...
#         self.Input_root_dir  = './patches/'
#         self.Target_root_dir = './noisy_patches/'
#
...
#
#     Input_root_dir = r'./patches/'
#     Target_root_dir = r'./noisy_patches/'
...
def irregular_mask(data, rate):
    H, W = data.shape[:2]
    mask = np.ones((H, W), dtype=data.dtype)
    cols = np.random.choice(W, size=int(rate * W), replace=False)
    mask[:, cols] = 0
    return mask

class MyDataset(Dataset):
    def __init__(self, root_dir, patch_size=(128,128), transform=None):
    ...
    def __getitem__(self, idx):
        # 1) Загружаем «чистый» патч
        path = os.path.join(self.root_dir, self.files[idx])
        data = np.load(path)               # shape (H, W)
        # 2) Генерируем маску и шум
        rate = random.choice([0.02, 0.08, 0.18, 0.28, 0.38])
        mask = irregular_mask(data, rate)
        data_masked = data * mask

        SNR = random.choice([-2, -1, 1, 5, 10])
        noise = np.random.randn(*data.shape)
        noise -= noise.mean()
        P = np.linalg.norm(data - data.mean())**2 / data.size
        var = P / (10 ** (SNR/10))
        noise = noise * (np.sqrt(var) / noise.std())
        data_noisy = data_masked + noise

        # 3) приводим к нужным формам (H,W,1) → Tensor C×H×W
        inp = data_noisy[..., None]
        tgt = data[..., None]
        if self.transform:
...
if __name__ == '__main__':
    ...
    print(x.shape, y.shape)  # (8,1,128,128) (8,1,128,128)
