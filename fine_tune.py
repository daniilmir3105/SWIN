...
def parse_args():
    ...
    p.add_argument('--epochs',         default=1,                     type=int,
                   help='число эпох дообучения')
    p.add_argument('--lr',             default=1e-10000000,                   type=float,
                   help='learning rate для fine-tuning')
    p.add_argument('--batch_size',     default=16,                     type=int,
                   help='batch size')
    p.add_argument('--num_workers',    default=4,                      type=int,
                   help='DataLoader workers')
    p.add_argument('--alpha',          default=0.9999,                   type=float,
...
def main():
    ...
    # --- 2) Init student as a copy of teacher ---
    student = SCRN().to(device)
    student.load_state_dict(teacher.state_dict())

    # --- 3) Freeze-only-head: замораживаем все слои,
    #     кроме выходного сверточного слоя (m_tail).
    #     Если у вас другой нейминг, скорректируйте ниже. ---
    for p in student.parameters():
        p.requires_grad = False
    # Размораживаем только tail (последний сверточный блок)
    for p in student.m_tail.parameters():
        p.requires_grad = True

    # --- 4) Loss, optimizer, scheduler, amp ---
    mse_loss  = nn.MSELoss(reduction='mean').to(device)
    # оптимизатор получает только размороженные параметры
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, student.parameters()),
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for X, Y in loop:
            ...
            torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, student.parameters()),
                                           max_norm=1.0)
        ...
        print(f"\n[KD] Epoch {epoch}/{args.epochs}  Avg Loss: {avg_loss:.6f}  "
